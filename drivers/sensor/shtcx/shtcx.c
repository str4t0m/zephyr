...
/* all cmds read temp first: cmd[MODE][Clock_stretching_enabled] */
static const uint16_t measure_cmd[2][2] = {
	{ 0x7866, 0x7CA2 },
	{ 0x609C, 0x6458 },
};

/* measure_wait_us[shtcx_chip][MODE] */
			       ...
			       enum sensor_channel chan)
{
	...
	if (shtcx_read_words(dev, measure_cmd[SHTCX_MODE][cfg->clock_stretching],
			     (uint16_t *)&data->sample, 2,
			     measure_wait_us[cfg->chip][SHTCX_MODE]) < 0) {
...
#define SHTCX_CONFIG(inst)						\
	{								\
		...
		.chip = TO_CHIP(inst),					\
		.clock_stretching =					\
			DT_INST_PROP_OR(inst, clock_stretching, 0U),	\
	}
